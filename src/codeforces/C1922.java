...
public class C1922 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        while (t-- > 0) {
            ...
            long[] mins0 = new long[n];
            int i = 1;
            while (i != n) {
                if (closest[i-1] != i) {
                    mins0[i] = mins0[i-1] + a[i] - a[i-1];
                } else {
                    mins0[i] = mins0[i-1] + 1;
                }
                i++;
            }
            long[] mins1 = new long[n];
            i = n-2;
            while (i != -1) {
                if (closest[i+1] != i) {
                    mins1[i] = mins1[i+1] + a[i+1] - a[i];
                } else {
                    mins1[i] = mins1[i+1] + 1;
                }
                i--;
            ...
            while (m-- > 0) {
                int x = fastScanner.nextInt() - 1;
                int y = fastScanner.nextInt() - 1;
                if (x < y) {
                    fastOutput.println(mins0[y] - mins0[x]);
                } else {
                    fastOutput.println(mins1[y] - mins1[x]);
                }
            }
        }
        fastOutput.close();

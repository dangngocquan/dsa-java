...
public class A1922 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        while (t-- > 0) {
            ...
            char[] a = fastScanner.next().toCharArray();
            char[] b = fastScanner.next().toCharArray();
            char[] c = fastScanner.next().toCharArray();
            fastOutput.println(solve(a, b, c));
        }
        fastOutput.close();
    }

    public static String solve(char[] a, char[] b, char[] c) {
        char[] t = new char[a.length];
        Arrays.fill(t, '-');
        for (int i = 0; i < a.length; i++) {
            if (a[i] == b[i]) {
                t[i] = a[i];
                if (c[i] != t[i]) return "YES";
            }
        }
        for (int i = 0; i < c.length; i++) {
            if (t[i] == '-') {
                if (c[i] != a[i] && c[i] != b[i]) return "YES";
            }
        }
        return "NO";
    }

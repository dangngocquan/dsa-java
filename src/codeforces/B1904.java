...
public class B1904 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        while (t-- > 0) {
            ...
            Map<Long, Integer> mapIndex = new TreeMap<>();
            Map<Integer, Long> mapKey = new TreeMap<>();
            int j = 0;
            ...
            j = 0;
            long[] dpSum = new long[map.size()];
            long[] dpCount = new long[map.size()];
            for (Long key : map.keySet()) {
                dpSum[j] = j > 0? dpSum[j-1] + key * map.get(key) : key * map.get(key);
                dpCount[j] = j > 0? dpCount[j-1] + map.get(key) : map.get(key);
                j++;
            }
            long[] ans = new long[map.size()];
            ans[map.size() - 1] = dpCount[map.size()-1];
            for (int i = map.size() - 2; i >= 0; i--) {
                if (dpSum[i] >= mapKey.get(i+1)) {
                    ans[i] = ans[i+1];
                } else {
                    ans[i] = dpCount[i];
                }
            ...
            for (int i = 0; i < n; i++) {
                int index = mapIndex.get(a[i]);
                fastOutput.print(ans[index] - 1);
                fastOutput.print(" ");
            }
            fastOutput.println("");
        }

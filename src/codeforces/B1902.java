...
public class B1902 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        while (tc-- > 0) {
            ...
            long P = fastScanner.nextLong();
            long l = fastScanner.nextLong();
            long t = fastScanner.nextLong();

            long n1 = n-8 >= 0? (n-8) / 14 + 1 : 0;
            long n2 = (n <= 7 || (n > 7 && n % 14 <= 7 && n % 14 > 0))? 1 : 0;
            long n3 = n - n1 - n2;
            if (n1 * (l + 2 * t) >= P) {
                long p0 = l + 2 * t;
                long k0 = P / p0;
                fastOutput.println(P % p0 == 0? n - k0 : n - k0 - 1);
                continue;
            }
            P -= n1 * (l + 2 * t);
            if (n2 * (l + t) >= P) {
                fastOutput.println(n - n1 - 1);
                continue;
            }
            P -= n2 * (l + t);
            if (n3 * l >= P) {
                long k0 = P / l;
                fastOutput.println(P % l == 0? n - n1 - n2 - k0 : n - n1 - n2 - k0 - 1);
            }
        }
        fastOutput.close();
    }

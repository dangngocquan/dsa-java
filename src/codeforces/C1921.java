...
public class C1921 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        while (t-- > 0) {
            ...
            long f = fastScanner.nextLong();
            long a = fastScanner.nextLong();
            long b = fastScanner.nextLong();
            long[] m = new long[n+1];
            for (int i = 1; i <= n; i++) m[i] = fastScanner.nextLong();
            fastOutput.println(solve(n, f, a, b, m)? "YES" : "NO");
        }
        fastOutput.close();
    }

    public static boolean solve(int n, long f, long a, long b, long[] m) {
        long f0 = 0;
        for (int i = 0; i < n; i++) {
            long abs = m[i+1] - m[i];
            f0 += Math.min(a * abs, b);
            if (f0 >= f) return false;
        }
        return f0 < f;
    }
}
